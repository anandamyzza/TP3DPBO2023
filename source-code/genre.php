<?php

include('config/db.php');
include('classes/DB.php');
include('classes/Genre.php');
include('classes/Template.php');

/* Membuat Instance Genre, Membuka Koneksi, dan Menampilkan Data Album */
$genre = new Genre($DB_HOST, $DB_USERNAME, $DB_PASSWORD, $DB_NAME);
$genre->open();
$genre->getGenre();

/* SEARCH DATA GENRE */
if (isset($_POST['btn-search']))
{
    $genre->searchGenre($_POST['search']);
}
else
{
    $genre->getGenre();
}

/* DISPLAY DATA GENRE */
$view = new Template('templates/skintabel.html');
$mainTitle = 'Genre';
$header = '<tr>
<th scope="row">No.</th>
<th scope="row">Genre Name</th>
<th scope="row">Action</th>
</tr>';
$data = null;
$no = 1;
$formLabel = 'Genre';

while ($div = $genre->getResult())
{
    $data .= '<tr>
    <th scope="row">' . $no . '</th>
    <td>' . $div['genre_name'] . '</td>
    <td style="font-size: 22px;">
        <a href="genre.php?id=' . $div['genre_id'] . '" title="Edit Data"><i class="bi bi-pencil-square text-warning"></i></a>
        &nbsp;
        <a href="genre.php?delete=' . $div['genre_id'] . '" title="Delete Data"><i class="bi bi-trash-fill text-danger"></i></a>
        </td>
    </tr>';
    $no++;
}

/* ADD DATA GENRE */
if (!isset($_GET['id']))
{
    if (isset($_POST['submit']))
    {
        if ($genre->addGenre($_POST) > 0)
        {
            echo "<script>
                alert('Data berhasil ditambah!');
                document.location.href = 'genre.php';
            </script>";
        }
        else
        {
            echo "<script>
                alert('Data gagal ditambah!');
                document.location.href = 'genre.php';
            </script>";
        }
    }

    $title = 'Add';
    $type = '<button type="submit" class="btn btn-success" name="submit">Add Data</button>';
}

/* UPDATE DATA GENRE */
if (isset($_GET['id']))
{
    $id = $_GET['id'];
    if ($id > 0)
    {
        if (isset($_POST['submit']))
        {
            if ($genre->updateGenre($id, $_POST) > 0)
            {
                echo "<script>
                    alert('Data berhasil diubah!');
                    document.location.href = 'genre.php';
                </script>";
            }
            else
            {
                echo "<script>
                    alert('Data gagal diubah!');
                    document.location.href = 'genre.php';
                </script>";
            }
        }

        $genre->getGenreById($id);
        $row = $genre->getResult();

        $dataUpdate = $row['genre_name'];
        $title = 'Edit';
        $type = '<button type="submit" class="btn btn-primary" name="submit">Edit Data</button>';

        $view->replace('DATA_VAL_UPDATE', $dataUpdate);
    }
}

/* DELETE DATA GENRE */
if (isset($_GET['delete']))
{
    $id = $_GET['delete'];
    if ($id > 0)
    {
        if ($genre->deleteGenre($id) > 0)
        {
            echo "<script>
                alert('Data berhasil dihapus!');
                document.location.href = 'genre.php';
            </script>";
        }
        else
        {
            echo "<script>
                alert('Data gagal dihapus!');
                document.location.href = 'genre.php';
            </script>";
        }
    }
}

$genre->close();

$view->replace('DATA_MAIN_TITLE', $mainTitle);
$view->replace('DATA_TABEL_HEADER', $header);
$view->replace('DATA_TITLE', $title);
$view->replace('BUTTON_TYPE', $type);
$view->replace('DATA_FORM_LABEL', $formLabel);
$view->replace('DATA_TABEL', $data);
$view->write();
